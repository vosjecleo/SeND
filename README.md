...
SeND uses Matrix Spaces as Discord-style 'servers', Matrix rooms as text 'channels' and DMs, and MatrixRTC for voice/video communication.
...
- SeND's GIF search proxy / KLIPY when using GIF search (older saved GIPHY
...
SeND stores account/session information in the operating system's application-data locations and uses secure storage where appropriate.
...
SeND has currently been built and/or tested on:
...
SeND would not exist without the Matrix ecosystem and the open-source projects it builds upon.

See [CREDITS.md](CREDITS.md) for dependency, library, font, dataset and project acknowledgements.

See [LICENSE](LICENSE) for SeND's license and
